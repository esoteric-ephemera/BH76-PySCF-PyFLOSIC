...
opt_type = {
    'float': ['tol','max_inc','fmax','levelshift'],
    'int': ['gridsize','charge','2S','verbose','max_step','max_cycle'],
    'bool': ['symm'],
    'str': ['xyz_fl','basis','xc','ecp','logfile','opt','label']
}
...
def fod_opt():
    ...
    dopts = {
        'gridsize': 3, 'basis': 'DFO', 'symm': False,
        'tol': 1.e-6, 'charge': 0, '2S': 0,
        'xc': 'LDA, PW', 'verbose': 4, 'ecp' : {}, 'opt': 'FIRE',
        'max_inc' : 0.1, 'label': 'OPT_FRMORB', 'fmax' : 1.e-3,
        'max_step' : 1000, 'max_cycle': 500, 'levelshift' : 0.0
        }
    ...
    if dopts['basis'] in ['DFO','DFO+','UNC_DFO','UNC_DFO+']:
        tdict = get_ats_from_xyz(dopts['xyz_fl'])
        tstr = ''
        for at in tdict:
            tstr += at
    ...
    pycom_guess(ase_nuclei=aio.read(dopts['xyz_fl']), charge = dopts['charge'], \
        ...
        verbose = dopts['verbose'], levelshift=dopts['levelshift'])
